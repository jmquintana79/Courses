...
# Second Constrain: here is the difference ("*n[1]") between de previous SCOP and this NCQP problem.
model.C2 = pyo.Constraint(expr = C[2] == 6*n[2]*n[1]) 
model.C3 = pyo.Constraint(expr = C[3] == 7*n[3])

#solve
opt = SolverFactory('gurobi') # despite of "gurobi" is a linear solver
opt.options['NonConvex'] = 2  # HERE is the way to allow gurobi solves this kind of problema. 
opt.solve(model)
