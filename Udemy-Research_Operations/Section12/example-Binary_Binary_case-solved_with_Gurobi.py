...
model.C2n = pyo.Constraint(expr = n[2] <= b[1]*b[2]*1000) # non-linear constrain bin*bin
model.C3 = pyo.Constraint(expr = C[3] == 7*n[3])

#solve
opt = SolverFactory('gurobi') # este solver lo resuelve automaticamente sin tener que linealizar
opt.solve(model)
